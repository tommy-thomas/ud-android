...
//      TODO (7) Generate a weather summary with the date, description, high and low
        int INDEX_WEATHER_DATE = 1;
        int INDEX_WEATHER_ID = 1;
        int INDEX_HIGH_TEMP = 1;
        int INDEX_LOW_TEMP = 1;
        long dateMillis = mCursor.getLong(INDEX_WEATHER_DATE);
        String dateString = SunshineDateUtils.getFriendlyDateString(mContext, dateMillis, false);
        int weatherID = mCursor.getInt(INDEX_WEATHER_ID);
        double highCelcius = mCursor.getDouble(INDEX_HIGH_TEMP);
        double lowCelcius = mCursor.getDouble(INDEX_LOW_TEMP);
        String weatherSummary = dateString;// + " - " + description + " - " + highAndLowTemperature;

//      TODO (8) Display the summary that you created above
        forecastAdapterViewHolder.weatherSummary.setText(dateString);
    }
